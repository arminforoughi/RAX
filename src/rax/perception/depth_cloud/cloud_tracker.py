...
from rax.models.depth.stereo import StereoDepth, clamp_roi
from rax.models.detection import Detection, MaskTracker, PromptDetector
from rax.perception.camera_interface import DepthSource, StereoDepthSource
from rax.perception.depth_cloud.backproject import (
    backproject_masked,
    box_iou,
    box_mask,
    box_to_roi,
...
from rax.perception.depth_cloud.stream import CloudUpdate, PointCloudStream
...
class CloudTracker:
    def __init__(
        ...
        stereo: StereoDepth | DepthSource,
        query: str,
        *,
        ...
        # Either a raw stereo matcher (the historical argument) or any DepthSource.
        # An RGB-D rig passes SensorDepthSource and no matcher ever runs; a mono rig
        # passes NoDepthSource and the cloud path quietly stands down — see below.
        # `available` is the honest discriminator: both protocols expose `name` and
        # `depth_meters`, so an isinstance() check against either would match both.
        self.depth: DepthSource = (
            stereo if hasattr(stereo, "available") else StereoDepthSource(stereo))
    ...
    def _note_no_depth(self) -> None:
        """Say once that this rig has no depth, then stay quiet about it."""
        if self._warned_no_depth:
            return
        logger.info(
            "[cloud] %s supplies no depth on this rig — point clouds are off; "
            "localization runs on the plane-ray and apparent-size strategies instead",
            self.depth.name)
    ...
    def _update_clouds(self, obs: Observation) -> list[CloudUpdate]:
        ...
        if not self.depth.available:
            # Checked before the loop, not inside it: segmenting an object to a precise
            # SAM2 mask and only then discovering there is nothing to back-project
            # would burn the most expensive call in the tick for no result.
            self._note_no_depth()
            return updates
        for tag in self._scheduled_tags():
            ...
            depth = self.depth.depth_meters(obs.frame, roi=roi)
            if depth is None:
                # Claimed available but delivered nothing — a frame missing its right
                # image, say. Same outcome, but worth not silently retrying per tag.
                self._note_no_depth()
                break
