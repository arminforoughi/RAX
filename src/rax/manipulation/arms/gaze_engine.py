...
from rax.manipulation.arms.arm_interface import ArmInterface, Observation
from rax.manipulation.arms.grasp import GraspConfig, close_with_current, release
from rax.manipulation.arms.kinematics import Kinematics
from rax.manipulation.arms.se3 import look_at_R, look_at_ray_R, make_pose, rate_limited_step
from rax.models.depth.stereo import StereoIntrinsics
from rax.perception.locate import ELONGATED_ASPECT

if TYPE_CHECKING:
    from rax.perception.depth_cloud import CloudTracker, ObjectTrack
...
class GazeConfig:
    ...
    # Fall back to apparent-size ranging when the rig produces no depth at all. This
    # is what makes a wrist-mounted webcam a working rig rather than an unsupported
    # one. Turn it off to force a hard failure instead of a weaker estimate.
    apparent_size_fallback: bool = True
...
class GazeEngine:
    def __init__(
        ...
        *,
        ...
        self.kin = kinematics
        self.cloud = cloud
        # Class size priors, for the apparent-size range fallback. A rig with no depth
        # sensor has these and nothing else; see _depth_from_apparent_size.
        if priors is None:
    ...
    def _depth_from_apparent_size(self, focus: ObjectTrack, obs: Observation) -> float | None:
        """Range from how big the object looks — the only range a mono rig can get.

        ``z = fx * W / w_px``, with ``W`` the class's characteristic width. This is
        AXIAL DEPTH, which is exactly what :meth:`_filter_depth` wants, so no
        conversion is needed here (see ``CameraGeometry.depth_from_width`` for why the
        distinction matters elsewhere).

        Weaker than a depth map and honest about it: the estimate is only as good as
        the size prior, and for an elongated object seen end-on versus side-on the
        bbox width swings by the aspect ratio. It is refused above
        :data:`~rax.perception.locate.ELONGATED_ASPECT` rather than returning a number
        that would drive the arm at the wrong moment — an under-estimated range makes
        the gripper close on air, which is the worse failure of the two.
        """
        box = getattr(focus, "box", None)
        if not self.cfg.apparent_size_fallback or self.priors is None or box is None:
            return None
        x1, y1, x2, y2 = (float(v) for v in box)
        w_px, h_px = max(1.0, x2 - x1), max(1.0, y2 - y1)
        if max(w_px, h_px) / min(w_px, h_px) > ELONGATED_ASPECT:
            return None
        width_m = float(self.priors.size_m(getattr(focus, "label", "")))
        if width_m <= 1e-3:
            return None
        return float(obs.intrinsics.fx) * width_m / w_px
    ...
    def _depth_from_focus(self, focus: ObjectTrack, obs: Observation) -> float | None:
        ...
        z = self._depth_from_apparent_size(focus, obs)
        if z is not None:
            return self._filter_depth(z)
        return self._depth_filt
    ...
    def _servo_approach(self, obs: Observation, focus: ObjectTrack, dt: float) -> None:
        ...
        # A cloud is the strong evidence. A rig with no depth sensor never produces
        # one, so apparent-size range is promoted to "reliable" there — otherwise the
        # latch never forms, and the approach runs forever on the 0.25 m placeholder
        # above instead of on a measurement.
        has_cloud = (focus.has_cloud
                     and focus.points.shape[0] >= self.cfg.cloud_min_pts_depth)
        sized_only = (not getattr(self.cloud.depth, "available", True)
                      and depth is not None)
        reliable = (
            (has_cloud or sized_only)
