...
Dev harness (no hardware, no model weights)::
    ...
    python -m rax.manipulation.arms.run_gaze --backend mock --query "red box" --place-on "blue box"

Real SO-101 + OAK-D (needs the arm, the camera, and RAFT/FoundationStereo weights)::

    python -m rax.manipulation.arms.run_gaze --backend so101 --interface eth0 \
...
from rax.perception.depth_cloud.stream import CloudUpdate
...
def main() -> None:
    ...
    viz = None
    if args.rerun:
        from rax.perception.depth_cloud.rerun_viz import RerunViz
    ...
    if viz:
        ...
        from rax.manipulation.arms.gaze_engine import DONE, FAILED
