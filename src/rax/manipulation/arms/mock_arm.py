...
from rax.manipulation.arms.kinematics import CartesianKinematics, pose_to_xyzrpy
...
def table_scene(table_z: float = 0.0) -> list[MockObject]:
    """The same three objects, but standing on a table in a Z-UP base frame.

    :func:`default_scene` is expressed in the mock arm's own base frame, which follows
    the OpenCV camera convention (Y down, Z forward) because the mock arm IS its
    camera. A robot with a camera somewhere else has an ordinary Z-up base frame with
    the table at ``z = table_z``, and the same coordinates there would put the objects
    floating half a metre in the air behind the robot.

    Each object's centre sits one radius above the table, which is where a sphere
    resting on a surface actually is — the plane-ray localizer solves for exactly that
    relationship, so getting it wrong here would silently flatter or break it.
    """
    specs = [
        (0.26, -0.07, 0.030, (225, 40, 40), "red object"),
        (0.30, 0.06, 0.042, (40, 200, 70), "green object"),
        (0.22, 0.11, 0.045, (40, 70, 225), "blue object"),
    ]
    return [MockObject(np.array([x, y, table_z + r]), r, colour, label)
            for x, y, r, colour, label in specs]
...
class MockArmBody:
    """An arm with joints and a gripper and NO camera — the other half of a ``Rig``.

    :class:`MockArm` renders its own stereo pair because it models an eye-in-hand rig.
    Most arms are not like that: a lerobot follower, a UR5, an xArm all move and report
    joints while some entirely separate device does the seeing. This is that shape, so
    :class:`manipulation.arms.rig.Rig` can be tested against the contract real drivers
    actually have.

    The "joints" are the EE pose, as in :class:`CartesianKinematics`, so it needs no
    URDF and no solver backend.
    """

    joint_names = CartesianKinematics.joint_names

    def __init__(self, objects: list[MockObject] | None = None, *,
                 grasp_radius_m: float = 0.06, q0: np.ndarray | None = None):
        self.kin = CartesianKinematics()
        self.objects = list(objects if objects is not None else default_scene())
        self._q = (np.asarray(q0, dtype=np.float64).copy() if q0 is not None
                   else pose_to_xyzrpy(np.eye(4)))
        self._gripper = 100.0
        self._grasped: MockObject | None = None
        self._grasp_radius = float(grasp_radius_m)
    ...
    def get_state(self) -> ArmState:
        if self._grasped is not None:
            self._grasped.center = self.tool_xyz()
        return ArmState(joints_deg=self._q.copy(), gripper_pct=self._gripper)
    ...
    def set_gripper(self, pct: float) -> None:
        ...
        tool = self.tool_xyz()
        if pct < 50.0 and self._grasped is None:
            near = min(self.objects, key=lambda o: float(np.linalg.norm(o.center - tool)))
            if np.linalg.norm(near.center - tool) <= self._grasp_radius:
                self._grasped = near
        elif pct >= 80.0:
            self._grasped = None
    ...
    def read_gripper_current(self) -> float | None:
        return 100.0 + (220.0 if (self._grasped is not None and self._gripper < 80.0) else 0.0)

    # -- helpers ------------------------------------------------------------
    def tool_xyz(self) -> np.ndarray:
        return np.asarray(self.kin.forward_kinematics(self._q), dtype=np.float64)[:3, 3]

    @property
    def holding(self) -> MockObject | None:
