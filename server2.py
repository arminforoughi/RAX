  ...
  <div id="right">
    ...
    <div id="controls">
      ...
      <div class="btn-row">
        ...
        <button class="ctrl-btn btn-action" id="btn-fight" onclick="toggleFightMode()" style="background:#5a1a1a;">Fight Mode</button>
...
<script>
  ...
  let fightModeOn = false;
  function toggleFightMode() {
    fightModeOn = !fightModeOn;
    const btn = document.getElementById('btn-fight');
    if (fightModeOn) {
      cmd('fight_mode_on');
      btn.textContent = 'Fight Mode ON';
      btn.style.background = '#cc2200';
    } else {
      cmd('fight_mode_off');
      btn.textContent = 'Fight Mode';
...
<script>
  ...
  function toggleGuard() {
    ...
    if (guardOn) {
      ...
      status.textContent = 'guard up — ready to fight';
  ...
  document.addEventListener('keydown', (e) => {
    if (e.repeat) return;
    if (e.key === 'j' || e.key === 'J') punch('left');
    if (e.key === 'k' || e.key === 'K') punch('right');
    if (e.key === 'g' || e.key === 'G') toggleGuard();
  });
</script>
</body>
</html>"""
